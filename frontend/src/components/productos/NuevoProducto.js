// frontend/src/components/productos/NuevoProducto.js
...
const SERVICIOS = ["Odontología", "Vacunación", "Laboratorio", "Farmacia", "Urgencias", "Sueroterapia"];
...
const NuevoProducto = () => {
  ...
  const eliminarCategoria = async (cat) => {
    try {
      ...
      setFormData((prev) => (prev.categoria === cat._id ? { ...prev, categoria: "" } : prev));
    } catch (e) {
      console.error(e);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        productoBase: formData.productoBase.trim(),
        variante: formData.variante.trim(),
        nombre: nombreFinal, // backend también lo recalcula
        categoria: formData.categoria,
        tipoProducto: formData.tipoProducto.trim(),
        tipoServicio: formData.tipoServicio,
        unidadMedida: formData.unidadMedida,
        stockMinimo: Number.isFinite(Number(formData.stockMinimo)) ? Number(formData.stockMinimo) : 0,
  ...
  return (
    <Container fluid className="py-4">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2 className="fw-bold mb-0">Nuevo Producto</h2>
        ...
        <Modal.Body>
          ...
          <InputGroup className="mb-3">
            <Form.Control
              ...
              placeholder="Nueva categoría (Ej: Desinfectante)"
